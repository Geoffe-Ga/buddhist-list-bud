...
if command -v radon &> /dev/null; then
    ...
    radon cc -a buddhist_list_bud/ || true

    echo ""
    echo "Maintainability Index (should be >= 20):"
    radon mi -a buddhist_list_bud/ || true
else
...
if command -v xenon &> /dev/null; then
    ...
    xenon --max-absolute B --max-modules B --max-average B buddhist_list_bud/ ||         { echo "✗ Complexity exceeds thresholds" >&2; exit 1; }
else
    if $VERBOSE; then
